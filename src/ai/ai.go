...
const AI_REFLECTION_TIME time.Duration = time.Millisecond * 100
...
func Score(node Node) int {
	...
	opponentWithFullOfPebble := board.IsAPitWithMoreThanTwelvePebble(node.Board, node.Players[1-node.IndexCurrentPlayer])

	// Good for player
	opponentCountPebble := board.GetCountPitWithOneTwoPebble(node.Board, node.Players[1-node.IndexCurrentPlayer])
	playerWithFullOfPebble := board.IsAPitWithMoreThanTwelvePebble(node.Board, node.Players[node.IndexCurrentPlayer])
